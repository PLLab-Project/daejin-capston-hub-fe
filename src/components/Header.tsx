    ...
    <div className="sticky top-0 z-50">
      <header className="bg-brand text-white">
        <div className="page-container flex h-[35px] items-center justify-between md:h-[60px]">
          <a className="flex items-baseline gap-1" href="#/gallery" aria-label="졸업작품 갤러리 홈" onClick={(event) => handleNavItemClick(event, '갤러리')}>
      ...
      <nav className="hidden border-b border-neutral-200 bg-white shadow-sm md:block">
        <div className="page-container flex h-[40px] items-center">
          <div className="flex h-full items-center gap-12">
            {navItems.map((item) => (
              <a key={item} href={navHrefs[item]} onClick={(event) => handleNavItemClick(event, item)} className={`flex h-full items-center text-[13px] ${item === activeItem ? 'border-b-2 border-brand font-semibold text-neutral-900' : 'text-neutral-500 hover:text-brand'}`}>
                {item}
              </a>
            ))}
          </div>
      ...
      >
        <div className="min-h-0 overflow-hidden">
          {navItems.map((item) => (
            <a key={item} href={navHrefs[item]} onClick={(event) => handleNavItemClick(event, item)} className={`block px-6 py-3 text-[11px] ${item === activeItem ? 'bg-[#eef3ff] font-semibold text-neutral-900' : 'text-neutral-600'}`}>
              {item}
            </a>
          ))}
