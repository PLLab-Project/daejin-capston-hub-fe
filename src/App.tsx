...
export default function App() {
  ...
  return (
    <div id="top" className="flex min-h-screen flex-col bg-white text-neutral-800">
      <Header menuOpen={menuOpen} onMenuToggle={() => setMenuOpen((open) => !open)} onLoginClick={() => setAuthStep('login')} />
      <main className="page-container flex-1">
        ...
        {filteredProjects.length > 0 ? (
          <div className="grid grid-cols-1 gap-5 md:grid-cols-3 md:gap-x-10 md:gap-y-[37px] lg:grid-cols-4">
            {filteredProjects.map((project) => <ProjectCard key={project.id} project={project} onBookmark={toggleBookmark} />)}
          </div>
        ) : (
      ...
      <LoginModal
        ...
        onSubmit={() => setAuthStep(hasCompletedInitialProfile ? 'closed' : 'first-login')}
      />
      <FirstLoginModal
